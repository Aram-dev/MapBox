package com.aram_dev.mapbox.pluginsapp.activity.clustering
...
class GeoJsonClusteringActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapView?.getMapAsync { map: MapboxMap? ->
            ...
            val clusterLayers = arrayOf(
                intArrayOf(
                    150,
                    ResourcesCompat.getColor(resources, R.color.mapboxRed, theme)
                ),
                intArrayOf(
                    20,
                    ResourcesCompat.getColor(resources, R.color.mapboxGreen, theme)
                ),
                intArrayOf(
                    0,
                    ResourcesCompat.getColor(resources, R.color.mapboxBlue, theme)
                )
            )
            try {
                mapboxMap?.setStyle(
                    Style.Builder().apply {
                        ...
                        ResourcesCompat.getDrawable(resources, R.drawable.ic_baseline_track_changes, null)?.let {
                            withImage("icon-id", it)
                        }
                    }
                )
    ...
    private fun createClusterSource(): GeoJsonSource {
        return GeoJsonSource(
            "earthquakes", URI("asset://earthquakes.geojson"), GeoJsonOptions()
                .withCluster(true)
    ...
    private fun createClusterLevelLayer(level: Int, layerColors: Array<IntArray>): CircleLayer {
        ...
        toNumber(get("point_count")).run {
            circles.setFilter(
                if (level == 0) all(
                    has("point_count"),
                    gte(this, literal(layerColors[level][0]))
                ) else all(
                    has("point_count"),
                    gt(this, literal(layerColors[level][0])),
                    lt(this, literal(layerColors[level - 1][0]))
                )
            )
        }
        return circles
