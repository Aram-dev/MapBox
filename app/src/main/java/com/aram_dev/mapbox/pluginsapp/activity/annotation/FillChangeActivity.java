package com.aram_dev.mapbox.pluginsapp.activity.annotation;
  ...
  public void onMapReady(@NonNull MapboxMap map) {
    map.setStyle(new Style.Builder().fromUri(Style.MAPBOX_STREETS), style -> {
      ...
      fill = fillManager.create(new FillOptions()
        .withLatLngs(Config.STAR_SHAPE_POINTS)
        .withFillColor(ColorUtils.colorToRgbaString(Config.BLUE_COLOR))
      );
    });
  }
  ...
  public boolean onOptionsItemSelected(MenuItem item) {
    switch (item.getItemId()) {
      case R.id.action_id_alpha:
        fullAlpha = !fullAlpha;
        fill.setFillOpacity(fullAlpha ? Config.FULL_ALPHA : Config.PARTIAL_ALPHA);
        break;
      case R.id.action_id_visible:
        visible = !visible;
        fill.setFillOpacity(visible ? (fullAlpha ? Config.FULL_ALPHA : Config.PARTIAL_ALPHA) : Config.NO_ALPHA);
        break;
      case R.id.action_id_points:
        allPoints = !allPoints;
        fill.setLatLngs(allPoints ? Config.STAR_SHAPE_POINTS : Config.BROKEN_SHAPE_POINTS);
        break;
      case R.id.action_id_color:
        color = !color;
        fill.setFillColor(color ? Config.BLUE_COLOR : Config.RED_COLOR);
        break;
      case R.id.action_id_holes:
        holes = !holes;
        fill.setLatLngs(holes ? Config.STAR_SHAPE_HOLES : Config.STAR_SHAPE_POINTS);
        break;
      default:
